...
from bookapp.models import MainCategory, BookCategory, Book, SpecialCategory, WishList, Cart, CartItem, UserAccount
from bookapp.forms import UserAccountForm, CheckoutForm, CommentForm, LoginForm, RegistrForm
from bookapp.mixins import UserMixin, MyLoginRequiredMixin
...
def get_queryset_for_main_page(instance, slug):
    if slug:
        instance.special_category = SpecialCategory.objects.get(
            slug=slug)
...
# BookDetail
def save_comment_and_return_jsonresponce(instance, form):
...
def get_comment_order_by_id(instance):
...
def add_book_to_wishlist(instance, slug):
    if not instance.wishlist.books.filter(slug=slug).exists():
        book_model = Book.objects.get(slug=slug)
        instance.wishlist.books.add(book_model)


# DeleteFromWishList

def delete_book_from_wishlist(instance, slug):
...
def authenticate_and_login_user(request, username, password, message_text):
    ...
    if user:
        login(request, user)
        messages.add_message(request, messages.SUCCESS, mark_safe(message_text))
